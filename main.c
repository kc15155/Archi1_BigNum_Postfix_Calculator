      ...
      else
      {
         if (c=='+')
             ...
             {  maxdigits=a.number_of_digits+2;
                ...
                if (a.negative==1 && b.negative==1)
                 ...
                 for (int i=0 ; i<b.number_of_digits; i++)
                    newb[i+1]=b.digit[i];
                 b.digit=newb;
                 if (b.number_of_digits>a.number_of_digits)
                    {
                     c=b;
                     b=a;
                     a=c;
            ...
            for (int i=0; i<=a.number_of_digits ; i++)
             ...
             if (result.digit[0]=='0')
             {
                 while (result.digit[0]=='0' && result.number_of_digits>1)
                 {
                    result.number_of_digits--;
                    char * newdig = (char *) malloc (result.number_of_digits+1);
                    newdig[result.number_of_digits]='\0';
                    for (int i=0; i<result.number_of_digits; i++)
                        newdig[i]=result.digit[i+1];
                    result.digit=newdig;
                }
             }
             toadd = result;
            ...
            for (int i=0; i<(a.number_of_digits+b.number_of_digits) ; i++)
                result.digit[i]='0';
//             result.number_of_digits = mul (a.digit+(maxdigits-2), b.digit+(maxdigits-2), maxdigits-2, result.digit + (a.number_of_digits + b.number_of_digits -1));
         ...
         if (c=='/')
            ...
            if (output.digit[0]=='0')
                 {
                 while (output.digit[0]=='0' && output.number_of_digits>1)
                  {
                    output.number_of_digits--;
                    char * newdig = (char *) malloc (output.number_of_digits+1);
                    newdig[output.number_of_digits]='\0';
                    for (int i=0; i<output.number_of_digits; i++)
                        newdig[i]=output.digit[i+1];
                    output.digit=newdig;
                  }
                }
            toadd=output; 
